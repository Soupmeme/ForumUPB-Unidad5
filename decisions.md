...
### D24 — Estacion 6 autorada: una masa compacta unida por vinculos curvos
...
Esto informo la decision de color (Kiwi queria explorar la opcion de mezclar ambar+jade, pero pidio antes mirar que hizo el profesor): en vez de partir la masa en dos grupos visualmente distintos (lo cual adelantaria tres estaciones el reveal que le corresponde a la estacion 9, "una vision, dos generaciones"), la estacion 6 queda como UNA sola masa cohesionada, en un tono mezclado ambar+jade (via `sys.params.accent`, ya autorado en `stations.js` como 0.45). Conecta con el hilo generacional sin gastar el momento de la estacion 9.

Tecnica (`community-huddle`): masa unica, mas compacta que cualquier nube anterior (radio 0.62x), unida por ~26 vinculos de indice fijo que ahora SE CURVAN hacia el centro compartido (nueva capacidad: bonds de multiples segmentos siguiendo una curva cuadratica, `quadPoint`, la misma matematica de lerp-de-lerps que usa el referente) en vez de lineas rectas -- el "abrazo" que el estudio de movimiento documento para este estado especifico. Deliberadamente el opuesto de la textura amplia y recta que necesitara "confianza" (estacion 7) cuando se autore. `bondsMax` sube de 80 a 220 segmentos para dar lugar a bonds curvos multi-segmento en cualquier estacion futura que los quiera.
