...
### D26 — Estacion 7 autorada: la red crece una vez y se queda
...
Implementacion (`trust-grows`): reusa la MISMA maquinaria de bonds curvos de la estacion 6 (`quadPoint`), con `pull=0.08` (casi recto) en vez de 0.4, y muchos mas vinculos (40). El radio de la nube crece una sola vez usando `sys.activeTime` (tecnica de D18), de forma medida (~5s), y se queda expandida -- no vuelve a contraerse, igual que la estacion 3 nunca vuelve a cerrarse. Color: `hot` estable (a diferencia del `hot` titilante de los cruces de la estacion 6) -- la confianza es lo que CRISTALIZA de ese intercambio: proceso en la estacion 6, resultado en la 7.

Bug real encontrado y corregido de paso: `community-exchange` (estacion 6) calculaba la fase de sus estelas con `(elapsed - trailBack) % 1`, y el operador `%` de JavaScript no envuelve numeros negativos al rango [0,1) -- en el primer instante tras cargar la pagina, ese calculo podia dar negativo y producir `waypoints[-1]` (`undefined`), propagando `NaN` a esa estela. Se corrige con un modulo positivo explicito `((raw % 1) + 1) % 1`. Se aprovecha tambien para poner `Cache-Control: no-store` en `server.mjs`, porque el navegador podia reusar un `particleEngine.js` viejo entre recargas incluso con un query string nuevo en la URL, lo cual habia generado confusion al probar cambios anteriores.
