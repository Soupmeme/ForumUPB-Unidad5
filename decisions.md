...
### D28 — Estacion 7, cuarto intento: dos telarañas que laten juntas (SUPERADA por D29)
Se parte la poblacion en dos mitades (igual que la estacion 1 y 6: mayor a la izquierda, joven a la derecha), cada una arma su PROPIA telaraña (15 radios x 12 capas = la mitad de `sys.n`, contra los 30 radios de una sola tela en D27) -- mas suelta, menos anillos (2 en vez de 4), con un poco de ruido organico por particula (`baseR`/`phase`) para que no se lea como un diagrama tecnico sino como cohesion real. Cada tela usa el color de su generacion (`elder`/`young`), no `hot`.

El elemento nuevo: un latido COMPARTIDO y diminuto (`heartbeat`, amplitud de radio ~3.5%, mismo `sin(elapsed*1.1)` para ambos lados, sin desfase) que hace crecer y encoger ambas telas EN SINCRONIA.

Kiwi aclaro que esto no era lo pedido: dos estructuras separadas que laten en sincronia se leen como "una al lado de la otra", no como "juntas". El pedido real era que las dos poblaciones se fusionaran en UNA sola estructura -- ese fusionarse es, en palabras de Kiwi, "la premisa entera de la confianza entre ellas". Ver D29.

### D29 — Estacion 7, quinto intento: una sola telaraña compartida, entretejida (SUPERADA por D30)
Una unica tela (30 radios x 12 capas, como D27, pero ahora con significado generacional) en vez de dos. Los radios ALTERNAN de generacion alrededor del circulo (radio par = ambar, radio impar = jade) -- entretejidos en una misma rueda, no separados en mitades izquierda/derecha. Cada hilo radial se degrada de color: en el borde (lejos del centro) se ve el color puro de su generacion; segun se acerca al centro compartido, se mezcla hacia `hot` (el blanco calido ya establecido como "algo emergiendo/fusionado" en las estaciones 1, 5 y 6) -- en el centro mismo, todos los hilos convergen a blanco. El fusionarse queda literal: es un degrade a lo largo de cada hilo, no una paleta separada. Los anillos que cruzan radios de generaciones distintas heredan el color de cada vertice, asi que tambien se leen entretejidos (el color cambia de hilo a hilo alrededor del anillo).

Kiwi senalo que faltaba movimiento: la estructura final era correcta, pero sin una animacion que la lleve hasta ahi, "es la misma estructura de nuestra primera iteracion sin que pase nada". Pidio ver el proceso: arrancar como las dos masas distintas y ver como se reensamblan. Ver D30.

### D30 — Estacion 7 rehecha: animacion real, de dos masas a una tela tejida
Se vuelve una animacion de una sola vez (`sys.activeTime`, tecnica D18) con un arco real: empieza como dos masas organicas y separadas (ambar a la izquierda, jade a la derecha -- la misma imagen de apertura de la estacion 1), y cada particula MIGRA desde su posicion de masa hacia su lugar asignado en la tela compartida (el mismo layout final de D29: radio y capa segun indice, degrade de color hacia `hot` en el centro).

La migracion esta ESCALONADA por particula (reusa `sys.phase`, la misma tecnica de la estacion 5): cada particula empieza su propio viaje en un momento ligeramente distinto dentro de una ventana de ~3s, y tarda ~1.8s en llegar -- asi el reensamblado se ve como una cascada, no como un salto simultaneo. Los hilos (bonds) se dibujan durante TODO el trayecto, no solo al final: se ven estirarse y cruzar el espacio entre las dos masas mientras las particulas viajan, tensandose progresivamente segun el promedio de avance de sus dos extremos -- tenues mientras las masas siguen separadas, brillantes una vez que la tela se asienta. El color de cada particula tambien viaja: color puro de su generacion mientras esta separada, mezclandose hacia el degrade final solo a medida que llega. Una vez ensamblada, se sostiene en el latido compartido y diminuto de D29. Verificado que reingresar a la estacion reinicia la animacion completa.
...
### D31 — Estacion 8: "el camino" y "las nuevas rutas", un tronco ya hecho con ramas que se bifurcan
Estudiado del propio estado "routes" del referente (moment id literal `nuevas-rutas`, motion-study.md): unas pocas rutas curvas, cada una con su propia semilla, con una perla brillante que viaja en bucle sobre una bezier cuadratica desde un punto lejano hacia un centro compartido -- rutas discretas y legibles, nunca una masa indiferenciada. Se toma esa TECNICA (carriles fijos, curva cuadratica, brillo viajero) pero se invierte la geometria para nuestras dos clausulas: un tronco unico y ya terminado (ambar, quieto, presente por completo desde el instante en que la estacion se activa -- "la experiencia" ya construyo esto, no hay nada que observar volverse) y varias ramas nuevas (jade) que se bifurcan desde puntos reales y elegidos del tronco, creciendo hacia afuera cuando la estacion se activa (`sys.activeTime`, tecnica D18), escalonadas tanto entre ramas como particula por particula dentro de cada rama (el mismo recurso de las estaciones 5 a 7) -- el descubrimiento como algo que se ve pasar, no un estado ya terminado. Una vez crecida, un brillo viajero sigue recorriendo cada rama en bucle: el descubrir no se detiene, sigue mientras la estacion este activa.

Kiwi propuso la imagen de "una bifurcacion en el camino" antes de ver nada construido; el resultado coincide con esa lectura -- seis ramas, forma aceptada.

Ajuste tras la primera prueba visual: con 48 particulas y unos 23 vinculos compartiendo una curva corta, el mezclado aditivo saturaba los canales verde y azul, y las ramas se veian blanco-cian en vez de jade sin importar cuanto se bajara el brillo por particula (el problema era la cantidad de trazos superpuestos por pixel, no su brillo individual). Se resolvio reduciendo la densidad, no el brillo: solo 1 de cada 3 particulas de cada rama se renderiza (las demas quedan en el pool, en negro -- la misma tecnica de "visible" de la estacion 4), y los vinculos solo conectan esos mismos indices visibles, con el brillo por particula devuelto a un valor normal. El tronco no necesito el mismo ajuste: se vio ambar y estable desde la primera prueba.
