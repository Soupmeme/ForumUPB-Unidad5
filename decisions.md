...
Implementacion:
- `hand.js`: primero se autoro una mano procedural (tubos + elipsoide), pero no leia como una mano de verdad. Se reemplazo (a peticion de Kiwi, un solo intento) por un modelo real: "Low poly hands" de Anastasiia Ku (CC-BY, via Poly Pizza), en `assets/hand.glb`, cargado con GLTFLoader vendorizado. El GLB trae un par de manos; se conserva una sola (decision: una mano). Se re-materializa con la piel calida del proyecto y flat shading (el low-poly facetado lee como intencional en el museo abstracto). Se normaliza (centrada, tamano unitario) y `main.js` controla escala/posicion, subiendo hacia la masa. Atribucion obligatoria (CC-BY): ver `README`.
...
- Composicion (ajuste de Kiwi): la placa se hizo mas chica y se bajo para que quede como una etiqueta DEBAJO del exhibit; la mano y las particulas quedan claramente arriba, sobre la estacion. La mano alcanza la masa de costado (movimiento lateral), palma hacia arriba, con el pulgar hacia el lado contrario a la camara.
